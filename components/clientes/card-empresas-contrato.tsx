...
import { ModalAdicionarEmpresa } from "@/components/clientes/modal-adicionar-empresa";
...
export function CardEmpresasContrato({
  ...
  const podeAdicionar = podeAcessar(userRole, "adicionarEmpresa");

  const principal = empresas.find((e) => e.eh_principal) ?? empresas[0];
  const outras = principal ? empresas.filter((e) => e.id !== principal.id) : [];
